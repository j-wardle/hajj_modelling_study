...
# Filter for the year and month with the maximum arriving passengers
...
result_df <- iata_pilgrim_estimate %>%
  filter(year == '2018' & month == '08') %>% 
  rename(pilgrim_estimate = passengers) %>% 
  ungroup() %>% 
...
compare_proportions$method <- "iata_method2"
